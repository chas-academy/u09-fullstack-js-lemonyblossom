...
const LogChart = () => {
  ...
  const prepareData = () => {
    ...
    // Group logs by date and prepare data for chart
    ...
    const moodData = labels.map(date => groupedByDate[date].mood.reduce((a, b) => a + b, 0) / groupedByDate[date].mood.length); // Average mood
    const sleepData = labels.map(date => groupedByDate[date].sleep.reduce((a, b) => a + b, 0) / groupedByDate[date].sleep.length); // Average sleep
